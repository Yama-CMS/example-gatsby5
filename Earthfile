...
# Build the site
# https://www.gatsbyjs.com/docs/how-to/previews-deploys-hosting/deploying-to-gitlab-pages
build:
    FROM +node-base
    WORKDIR /workdir
    ...
    # Uncomment if you wish to disable telemetry. Learn more at https://www.gatsbyjs.com/docs/telemetry/
...
# Run the dev server from the host. Run locally with `earthly +serve`.
# You'll need node modules (get them with `earthly +update`).
# You might need to stop the container manually once you exit (`docker stop earthly_gatsby_dev_server`).
devserver:
    ARG port=8000
    ...
    LOCALLY
        WITH DOCKER --load earthly-dev-server-image=+node-base
            # We use `./node_modules/.bin/gatsby` instead of `npm run develop` so that we can pass flags.
            # We pass --host=0.0.0.0 because otherwise gatsby will try to bind to "localhost", which doesn't route to
            # 0.0.0.0 in the container's /etc/hosts. We could add a `--add-host` parameter to `docker run` instead.
            RUN docker run --rm \
                           --volume="$PWD:/workdir:Z" \
                           --publish [::1]:$port:8000 \
                           --name="$container_name" \
                           earthly-dev-server-image \
                           ./node_modules/.bin/gatsby develop --host=0.0.0.0 --port=8000
	END
